...
CONTAINS
   ...
   SUBROUTINE cpl_domain
      ...
      !
      nishape(1) = 1
      nishape(2) = nlei-nldi+1
      nishape(3) = 1
      nishape(4) = nlej-nldj+1
      !
      ! ... Allocate memory for data exchange
      !
      ALLOCATE(exfld(nlei-nldi+1, nlej-nldj+1, jpk), stat = nerror)      
      IF( nerror > 0 ) THEN
      ...
      CALL oasis_def_partition ( nid_part, paral, nerror, jpiglo*jpjglo )
      !
            ...
            DO jc = 1, ssnd(ktyp,ji)%nct
               DO jm = 1, kcplmodel
                  ...
                  IF( ssnd(ktyp,ji)%nid(jc,jm) /= -1 ) THEN
                     IF( ln_ctl ) WRITE(numout,*) "Variable defined in the namcouple"
                     isnd = isnd + 1
         ...
         DO jm = 1, ssnd(ktyp,kid)%ncplmodel
            ...
            IF( ssnd(ktyp,kid)%nid(jc,jm) /= -1 ) THEN   ! exclude halos from data sent to oasis
               ...
               IF( .NOT. ll3D ) THEN   ! send 2D or 3D fields
                  CALL oasis_put ( ssnd(ktyp,kid)%nid(jc,jm), kstep, pdata(nldi:nlei, nldj:nlej,jc), kinfo )
               ELSE 
                  CALL oasis_put ( ssnd(ktyp,kid)%nid(jc,jm), kstep, pdata(nldi:nlei, nldj:nlej,1:ssnd(ktyp,kid)%nlvl), kinfo )
               ENDIF
         ...
         DO jm = 1, srcv(ktyp,kid)%ncplmodel
            ...
            IF( srcv(ktyp,kid)%nid(jc,jm) /= -1 ) THEN
               ...
               IF( llaction ) THEN   ! data received from oasis do not include halos
                  kinfo = OASIS_Rcv
                  IF( ll_mask ) exfld(:,:,1:iu(1)) = exfld(:,:,1:iu(1)) * pmask(nldi:nlei,nldj:nlej,jm:jm)

                  IF( ll_1st ) THEN
                     pdata(nldi:nlei,nldj:nlej,ib:iu(2)) = exfld(:,:,1:iu(1))
                     ll_1st = .FALSE.
                  ELSE
                     pdata(nldi:nlei,nldj:nlej,ib:iu(2)) = pdata(nldi:nlei,nldj:nlej,ib:iu(2)) + exfld(:,:,1:iu(1))
                  ENDIF
