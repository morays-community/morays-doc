...
!!gm  lk_oasis is controlled by key_oasis3  ===>>>  It shoud be removed from the namelist
         WRITE(numout,*) '         OASIS coupling (atm or sas)                lk_oasis      = ', lk_oasis
...
!!gm abusive use of jp_none ??   ===>>> need to be check and changed by adding a jp_sas parameter
      ...
      IF( nn_components /= jp_iam_nemo ) THEN
         IF( nn_components == jp_iam_oce )   nn_fsbc = cpl_freq('O_SFLX',ntypsbc) / NINT(rn_Dt)
         IF( nn_components == jp_iam_sas )   nn_fsbc = cpl_freq('I_SFLX',ntypsbc) / NINT(rn_Dt)
         !
         IF(lwp)THEN
            WRITE(numout,*)
