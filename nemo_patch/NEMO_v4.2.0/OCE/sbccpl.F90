MODULE sbccpl
   ...
   INTEGER, PUBLIC              ::   OASIS_Sent        = -1
   INTEGER, PUBLIC              ::   OASIS_SentOut     = -1
   INTEGER, PUBLIC              ::   OASIS_ToRest      = -1
   INTEGER, PUBLIC              ::   OASIS_ToRestOut   = -1
#endif
   !                                   !!** namelist namsbc_cpl **
   TYPE ::   FLD_C                     !
...
!!       WRITE(numout,*)'      transmitted solar thru sea-ice  = ', TRIM(sn_rcv_qtrice%cldes), ' (', TRIM(sn_rcv_qtrice%clcat), ')'
      ...
      ! for each field: define the OASIS name                              (srcv(ntypsbc,:)%clname)
      !                 define receive or not from the namelist parameters (srcv(ntypsbc,:)%laction)
      !                 define the north fold type of lbc                  (srcv(ntypsbc,:)%nsgn)

      ! default definitions of srcv
      srcv(ntypsbc,:)%laction = .FALSE.   ;   srcv(ntypsbc,:)%clgrid = 'T'   ;   srcv(ntypsbc,:)%nsgn = 1.
      srcv(ntypsbc,:)%nct = 1   ;   srcv(ntypsbc,:)%nlvl = 1

      !                                                      ! ------------------------- !
      !                                                      ! ice and ocean wind stress !
      !                                                      ! ------------------------- !
      !                                                           ! Name
      srcv(ntypsbc,jpr_otx1)%clname = 'O_OTaux1'      ! 1st ocean component on grid ONE (T or U)
      srcv(ntypsbc,jpr_oty1)%clname = 'O_OTauy1'      ! 2nd   -      -         -     -
      srcv(ntypsbc,jpr_otz1)%clname = 'O_OTauz1'      ! 3rd   -      -         -     -
      srcv(ntypsbc,jpr_otx2)%clname = 'O_OTaux2'      ! 1st ocean component on grid TWO (V)
      srcv(ntypsbc,jpr_oty2)%clname = 'O_OTauy2'      ! 2nd   -      -         -     -
      srcv(ntypsbc,jpr_otz2)%clname = 'O_OTauz2'      ! 3rd   -      -         -     -
      !
      srcv(ntypsbc,jpr_itx1)%clname = 'O_ITaux1'      ! 1st  ice  component on grid ONE (T, F, I or U)
      srcv(ntypsbc,jpr_ity1)%clname = 'O_ITauy1'      ! 2nd   -      -         -     -
      srcv(ntypsbc,jpr_itz1)%clname = 'O_ITauz1'      ! 3rd   -      -         -     -
      srcv(ntypsbc,jpr_itx2)%clname = 'O_ITaux2'      ! 1st  ice  component on grid TWO (V)
      srcv(ntypsbc,jpr_ity2)%clname = 'O_ITauy2'      ! 2nd   -      -         -     -
      srcv(ntypsbc,jpr_itz2)%clname = 'O_ITauz2'      ! 3rd   -      -         -     -
      !
      ...
      !
      IF( TRIM( sn_rcv_tau%clvor ) == 'local grid' )   srcv(ntypsbc,jpr_otx1:jpr_itz2)%nsgn = -1.

      !                                                           ! Set grid and action
      SELECT CASE( TRIM( sn_rcv_tau%clvgrd ) )      !  'T', 'U,V', 'U,V,I', 'U,V,F', 'T,I', 'T,F', or 'T,U,V'
      CASE( 'T' )
         srcv(ntypsbc,jpr_otx1:jpr_itz2)%clgrid  = 'T'        ! oce and ice components given at T-point
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%laction = .TRUE.     ! receive oce components on grid 1
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%laction = .TRUE.     ! receive ice components on grid 1
      CASE( 'U,V' )
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%clgrid  = 'U'        ! oce components given at U-point
         srcv(ntypsbc,jpr_otx2:jpr_otz2)%clgrid  = 'V'        !           and           V-point
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%clgrid  = 'U'        ! ice components given at U-point
         srcv(ntypsbc,jpr_itx2:jpr_itz2)%clgrid  = 'V'        !           and           V-point
         srcv(ntypsbc,jpr_otx1:jpr_itz2)%laction = .TRUE.     ! receive oce and ice components on both grid 1 & 2
      CASE( 'U,V,T' )
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%clgrid  = 'U'        ! oce components given at U-point
         srcv(ntypsbc,jpr_otx2:jpr_otz2)%clgrid  = 'V'        !           and           V-point
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%clgrid  = 'T'        ! ice components given at T-point
         srcv(ntypsbc,jpr_otx1:jpr_otz2)%laction = .TRUE.     ! receive oce components on grid 1 & 2
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%laction = .TRUE.     ! receive ice components on grid 1 only
      CASE( 'U,V,I' )
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%clgrid  = 'U'        ! oce components given at U-point
         srcv(ntypsbc,jpr_otx2:jpr_otz2)%clgrid  = 'V'        !           and           V-point
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%clgrid  = 'I'        ! ice components given at I-point
         srcv(ntypsbc,jpr_otx1:jpr_otz2)%laction = .TRUE.     ! receive oce components on grid 1 & 2
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%laction = .TRUE.     ! receive ice components on grid 1 only
      CASE( 'U,V,F' )
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%clgrid  = 'U'        ! oce components given at U-point
         srcv(ntypsbc,jpr_otx2:jpr_otz2)%clgrid  = 'V'        !           and           V-point
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%clgrid  = 'F'        ! ice components given at F-point
         srcv(ntypsbc,jpr_otx1:jpr_otz2)%laction = .TRUE.     ! receive oce components on grid 1 & 2
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%laction = .TRUE.     ! receive ice components on grid 1 only
      CASE( 'T,I' )
         srcv(ntypsbc,jpr_otx1:jpr_itz2)%clgrid  = 'T'        ! oce and ice components given at T-point
      ...
      CASE( 'T,F' )
         ...
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%laction = .TRUE.     ! receive ice components on grid 1
      CASE( 'T,U,V' )
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%clgrid  = 'T'        ! oce components given at T-point
         srcv(ntypsbc,jpr_itx1:jpr_itz1)%clgrid  = 'U'        ! ice components given at U-point
         srcv(ntypsbc,jpr_itx2:jpr_itz2)%clgrid  = 'V'        !           and           V-point
         srcv(ntypsbc,jpr_otx1:jpr_otz1)%laction = .TRUE.     ! receive oce components on grid 1 only
         srcv(ntypsbc,jpr_itx1:jpr_itz2)%laction = .TRUE.     ! receive ice components on grid 1 & 2
      CASE default
      ...
      IF( TRIM( sn_rcv_tau%clvref ) == 'spherical' )   &           ! spherical: 3rd component not received
         &     srcv(ntypsbc, (/jpr_otz1, jpr_otz2, jpr_itz1, jpr_itz2/) )%laction = .FALSE.
      !
      IF( TRIM( sn_rcv_tau%clvor  ) == 'local grid' ) THEN        ! already on local grid -> no need of the second grid
            srcv(ntypsbc,jpr_otx2:jpr_otz2)%laction = .FALSE.
            srcv(ntypsbc,jpr_itx2:jpr_itz2)%laction = .FALSE.
            srcv(ntypsbc,jpr_oty1)%clgrid = srcv(ntypsbc,jpr_oty2)%clgrid   ! not needed but cleaner...
            srcv(ntypsbc,jpr_ity1)%clgrid = srcv(ntypsbc,jpr_ity2)%clgrid   ! not needed but cleaner...
      ENDIF
      !
      IF( TRIM( sn_rcv_tau%cldes ) /= 'oce and ice' ) THEN        ! 'oce and ice' case ocean stress on ocean mesh used
         srcv(ntypsbc,jpr_itx1:jpr_itz2)%laction = .FALSE.    ! ice components not received
         srcv(ntypsbc,jpr_itx1)%clgrid = 'U'                  ! ocean stress used after its transformation
         srcv(ntypsbc,jpr_ity1)%clgrid = 'V'                  ! i.e. it is always at U- & V-points for i- & j-comp. resp.
      ENDIF
      ENDIF
      ...
      srcv(ntypsbc,jpr_rain)%clname = 'OTotRain'      ! Rain = liquid precipitation
      srcv(ntypsbc,jpr_snow)%clname = 'OTotSnow'      ! Snow = solid precipitation
      srcv(ntypsbc,jpr_tevp)%clname = 'OTotEvap'      ! total evaporation (over oce + ice sublimation)
      srcv(ntypsbc,jpr_ievp)%clname = 'OIceEvap'      ! evaporation over ice = sublimation
      srcv(ntypsbc,jpr_sbpr)%clname = 'OSubMPre'      ! sublimation - liquid precipitation - solid precipitation
      srcv(ntypsbc,jpr_semp)%clname = 'OISubMSn'      ! ice solid water budget = sublimation - solid precipitation
      srcv(ntypsbc,jpr_oemp)%clname = 'OOEvaMPr'      ! ocean water budget = ocean Evap - ocean precip
      SELECT CASE( TRIM( sn_rcv_emp%cldes ) )
      CASE( 'none'          )       ! nothing to do
      CASE( 'oce only'      )   ;   srcv(ntypsbc,jpr_oemp)%laction = .TRUE.
      CASE( 'conservative'  )
         srcv(ntypsbc, (/jpr_rain, jpr_snow, jpr_ievp, jpr_tevp/) )%laction = .TRUE.
         IF( k_ice <= 1 )  srcv(ntypsbc,jpr_ievp)%laction = .FALSE.
      CASE( 'oce and ice'   )   ;   srcv(ntypsbc, (/jpr_ievp, jpr_sbpr, jpr_semp, jpr_oemp/) )%laction = .TRUE.
      ...
      !                                                      !     Runoffs & Calving     !
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_rnf   )%clname = 'O_Runoff'
      IF( TRIM( sn_rcv_rnf%cldes ) == 'coupled' ) THEN
         srcv(ntypsbc,jpr_rnf)%laction = .TRUE.
      ...
      ENDIF
      !
      srcv(ntypsbc,jpr_cal)%clname = 'OCalving'   ;  IF( TRIM( sn_rcv_cal%cldes) == 'coupled' )   srcv(ntypsbc,jpr_cal)%laction = .TRUE.
      srcv(ntypsbc,jpr_isf)%clname = 'OIcshelf'   ;  IF( TRIM( sn_rcv_isf%cldes) == 'coupled' )   srcv(ntypsbc,jpr_isf)%laction = .TRUE.
      srcv(ntypsbc,jpr_icb)%clname = 'OIceberg'   ;  IF( TRIM( sn_rcv_icb%cldes) == 'coupled' )   srcv(ntypsbc,jpr_icb)%laction = .TRUE.

      IF( srcv(ntypsbc,jpr_isf)%laction ) THEN
      ...
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_qnsoce)%clname = 'O_QnsOce'
      srcv(ntypsbc,jpr_qnsice)%clname = 'O_QnsIce'
      srcv(ntypsbc,jpr_qnsmix)%clname = 'O_QnsMix'
      SELECT CASE( TRIM( sn_rcv_qns%cldes ) )
      CASE( 'none'          )       ! nothing to do
      CASE( 'oce only'      )   ;   srcv(ntypsbc,               jpr_qnsoce   )%laction = .TRUE.
      CASE( 'conservative'  )   ;   srcv(ntypsbc, (/jpr_qnsice, jpr_qnsmix/) )%laction = .TRUE.
      CASE( 'oce and ice'   )   ;   srcv(ntypsbc, (/jpr_qnsice, jpr_qnsoce/) )%laction = .TRUE.
      CASE( 'mixed oce-ice' )   ;   srcv(ntypsbc,               jpr_qnsmix   )%laction = .TRUE.
      ...
      !                                                      !    solar radiation        !   Qsr
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_qsroce)%clname = 'O_QsrOce'
      srcv(ntypsbc,jpr_qsrice)%clname = 'O_QsrIce'
      srcv(ntypsbc,jpr_qsrmix)%clname = 'O_QsrMix'
      SELECT CASE( TRIM( sn_rcv_qsr%cldes ) )
      CASE( 'none'          )       ! nothing to do
      CASE( 'oce only'      )   ;   srcv(ntypsbc,               jpr_qsroce   )%laction = .TRUE.
      CASE( 'conservative'  )   ;   srcv(ntypsbc, (/jpr_qsrice, jpr_qsrmix/) )%laction = .TRUE.
      CASE( 'oce and ice'   )   ;   srcv(ntypsbc, (/jpr_qsrice, jpr_qsroce/) )%laction = .TRUE.
      CASE( 'mixed oce-ice' )   ;   srcv(ntypsbc,               jpr_qsrmix   )%laction = .TRUE.
      ...
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_dqnsdt)%clname = 'O_dQnsdT'
      IF( TRIM( sn_rcv_dqnsdt%cldes ) == 'coupled' )   srcv(ntypsbc,jpr_dqnsdt)%laction = .TRUE.
      !
      ...
      !                                                      !      10m wind module      !
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_w10m)%clname = 'O_Wind10'   ;   IF( TRIM(sn_rcv_w10m%cldes  ) == 'coupled' )   srcv(ntypsbc,jpr_w10m)%laction = .TRUE.
      !
      !                                                      ! ------------------------- !
      !                                                      !   wind stress module      !
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_taum)%clname = 'O_TauMod'   ;   IF( TRIM(sn_rcv_taumod%cldes) == 'coupled' )   srcv(ntypsbc,jpr_taum)%laction = .TRUE.
      !
      !                                                      ! ------------------------- !
      !                                                      !      Atmospheric CO2      !
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_co2 )%clname = 'O_AtmCO2'
      IF( TRIM(sn_rcv_co2%cldes   ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_co2 )%laction = .TRUE.
         l_co2cpl = .TRUE.
         IF(lwp) WRITE(numout,*)
      ...
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_mslp)%clname = 'O_MSLP'     ;   IF( TRIM(sn_rcv_mslp%cldes  ) == 'coupled' )    srcv(ntypsbc,jpr_mslp)%laction = .TRUE.
      !
      !                                                      ! --------------------------------- !
      !                                                      !  ice topmelt and conduction flux  !   
      !                                                      ! --------------------------------- !
      srcv(ntypsbc,jpr_topm )%clname = 'OTopMlt'
      srcv(ntypsbc,jpr_botm )%clname = 'OBotMlt'
      IF( TRIM(sn_rcv_iceflx%cldes) == 'coupled' ) THEN
         IF( TRIM( sn_rcv_iceflx%clcat ) == 'yes' ) THEN
            srcv(ntypsbc,jpr_topm:jpr_botm)%nct = nn_cats_cpl
         ELSE
            CALL ctl_stop( 'sbc_cpl_init: sn_rcv_iceflx%clcat should always be set to yes currently' )
         ENDIF
         srcv(ntypsbc,jpr_topm:jpr_botm)%laction = .TRUE.
      ENDIF
!!      !                                                      ! --------------------------- !
!!      !                                                      ! transmitted solar thru ice  !   
!!      !                                                      ! --------------------------- !
!!      srcv(ntypsbc,jpr_qtrice)%clname = 'OQtr'
!!      IF( TRIM(sn_rcv_qtrice%cldes) == 'coupled' ) THEN
!!         IF ( TRIM( sn_rcv_qtrice%clcat ) == 'yes' ) THEN
!!            srcv(ntypsbc,jpr_qtrice)%nct = nn_cats_cpl
!!         ELSE
!!           CALL ctl_stop( 'sbc_cpl_init: sn_rcv_qtrice%clcat should always be set to yes currently' )
!!         ENDIF
!!         srcv(ntypsbc,jpr_qtrice)%laction = .TRUE.
!!      ENDIF
      !                                                      ! ------------------------- !
      !                                                      !    ice skin temperature   !
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_ts_ice)%clname = 'OTsfIce'    ! needed by Met Office
      IF( TRIM( sn_rcv_ts_ice%cldes ) == 'ice' )   srcv(ntypsbc,jpr_ts_ice)%laction = .TRUE.
      IF( TRIM( sn_rcv_ts_ice%clcat ) == 'yes' )   srcv(ntypsbc,jpr_ts_ice)%nct     = nn_cats_cpl
      IF( TRIM( sn_rcv_emp%clcat    ) == 'yes' )   srcv(ntypsbc,jpr_ievp)%nct       = nn_cats_cpl

#if defined key_si3
      IF( ln_cndflx .AND. .NOT.ln_cndemulate ) THEN
         IF( .NOT.srcv(ntypsbc,jpr_ts_ice)%laction )  &
            &   CALL ctl_stop( 'sbc_cpl_init: srcv(ntypsbc,jpr_ts_ice)%laction should be set to true when ln_cndflx=T' )
      ENDIF
#endif
      !                                                      ! ------------------------- !
      !                                                      !      Wave breaking        !
      !                                                      ! ------------------------- !
      srcv(ntypsbc,jpr_hsig)%clname  = 'O_Hsigwa'    ! significant wave height
      IF( TRIM(sn_rcv_hsig%cldes  ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_hsig)%laction = .TRUE.
         cpl_hsig = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_phioc)%clname = 'O_PhiOce'    ! wave to ocean energy
      IF( TRIM(sn_rcv_phioc%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_phioc)%laction = .TRUE.
         cpl_phioc = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_sdrftx)%clname = 'O_Sdrfx'    ! Stokes drift in the u direction
      IF( TRIM(sn_rcv_sdrfx%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_sdrftx)%laction = .TRUE.
         cpl_sdrftx = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_sdrfty)%clname = 'O_Sdrfy'    ! Stokes drift in the v direction
      IF( TRIM(sn_rcv_sdrfy%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_sdrfty)%laction = .TRUE.
         cpl_sdrfty = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_wper)%clname = 'O_WPer'       ! mean wave period
      IF( TRIM(sn_rcv_wper%cldes  ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_wper)%laction = .TRUE.
         cpl_wper = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_wnum)%clname = 'O_WNum'       ! mean wave number
      IF( TRIM(sn_rcv_wnum%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_wnum)%laction = .TRUE.
         cpl_wnum = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_wstrf)%clname = 'O_WStrf'     ! stress fraction adsorbed by the wave
      IF( TRIM(sn_rcv_wstrf%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_wstrf)%laction = .TRUE.
         cpl_wstrf = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_wdrag)%clname = 'O_WDrag'     ! neutral surface drag coefficient
      IF( TRIM(sn_rcv_wdrag%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_wdrag)%laction = .TRUE.
         cpl_wdrag = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_charn)%clname = 'O_Charn'     ! Chranock coefficient
      IF( TRIM(sn_rcv_charn%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_charn)%laction = .TRUE.
         cpl_charn = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_bhd)%clname = 'O_Bhd'     ! Bernoulli head. waves' induced surface pressure
      IF( TRIM(sn_rcv_bhd%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_bhd)%laction = .TRUE.
         cpl_bhd = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_tusd)%clname = 'O_Tusd'     ! zonal stokes transport
      IF( TRIM(sn_rcv_tusd%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_tusd)%laction = .TRUE.
         cpl_tusd = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_tvsd)%clname = 'O_Tvsd'     ! meridional stokes tranmport
      IF( TRIM(sn_rcv_tvsd%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_tvsd)%laction = .TRUE.
         cpl_tvsd = .TRUE.
      ENDIF

      srcv(ntypsbc,jpr_twox)%clname = 'O_Twox'     ! wave to ocean momentum flux in the u direction
      srcv(ntypsbc,jpr_twoy)%clname = 'O_Twoy'     ! wave to ocean momentum flux in the v direction
      srcv(ntypsbc,jpr_tawx)%clname = 'O_Tawx'     ! Net wave-supported stress in the u direction
      srcv(ntypsbc,jpr_tawy)%clname = 'O_Tawy'     ! Net wave-supported stress in the v direction
      IF( TRIM(sn_rcv_taw%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_twox)%laction = .TRUE.
         srcv(ntypsbc,jpr_twoy)%laction = .TRUE.
         srcv(ntypsbc,jpr_tawx)%laction = .TRUE.
         srcv(ntypsbc,jpr_tawy)%laction = .TRUE.
         cpl_taw = .TRUE.
      ENDIF
      !
      !                                                      ! ------------------------------- !
      ...
      !                                                      ! ------------------------------- !
      srcv(ntypsbc,jpr_sflx)%clname = 'O_SFLX'
      srcv(ntypsbc,jpr_fice)%clname = 'RIceFrc'
      !
      IF( nn_components == jp_iam_oce ) THEN    ! OCE coupled to SAS via OASIS: force received field by OCE (sent by SAS)
         srcv(ntypsbc,:)%laction = .FALSE.   ! force default definition in case of opa <-> sas coupling
         srcv(ntypsbc,:)%clgrid  = 'T'       ! force default definition in case of opa <-> sas coupling
         srcv(ntypsbc,:)%nsgn    = 1.        ! force default definition in case of opa <-> sas coupling
         srcv(ntypsbc, (/jpr_qsroce, jpr_qnsoce, jpr_oemp, jpr_sflx, jpr_fice, jpr_otx1, jpr_oty1, jpr_taum/) )%laction = .TRUE.
         srcv(ntypsbc,jpr_otx1)%clgrid = 'U'        ! oce components given at U-point
         srcv(ntypsbc,jpr_oty1)%clgrid = 'V'        !           and           V-point
         ! Vectors: change of sign at north fold ONLY if on the local grid
         srcv(ntypsbc, (/jpr_otx1,jpr_oty1/) )%nsgn = -1.
      ...
      !                                                      ! -------------------------------- !
      srcv(ntypsbc,jpr_toce  )%clname = 'I_SSTSST'
      srcv(ntypsbc,jpr_soce  )%clname = 'I_SSSal'
      srcv(ntypsbc,jpr_ocx1  )%clname = 'I_OCurx1'
      srcv(ntypsbc,jpr_ocy1  )%clname = 'I_OCury1'
      srcv(ntypsbc,jpr_ssh   )%clname = 'I_SSHght'
      srcv(ntypsbc,jpr_e3t1st)%clname = 'I_E3T1st'
      srcv(ntypsbc,jpr_fraqsr)%clname = 'I_FraQsr'
      !
      IF( nn_components == jp_iam_sas ) THEN
         IF( .NOT. ln_cpl ) srcv(ntypsbc,:)%laction = .FALSE.   ! force default definition in case of opa <-> sas coupling
         IF( .NOT. ln_cpl ) srcv(ntypsbc,:)%clgrid  = 'T'       ! force default definition in case of opa <-> sas coupling
         IF( .NOT. ln_cpl ) srcv(ntypsbc,:)%nsgn    = 1.        ! force default definition in case of opa <-> sas coupling
         srcv(ntypsbc, (/jpr_toce, jpr_soce, jpr_ssh, jpr_fraqsr, jpr_ocx1, jpr_ocy1/) )%laction = .TRUE.
         srcv(ntypsbc, jpr_e3t1st )%laction = .NOT.ln_linssh
         srcv(ntypsbc,jpr_ocx1)%clgrid = 'U'        ! oce components given at U-point
         srcv(ntypsbc,jpr_ocy1)%clgrid = 'V'        !           and           V-point
         ! Vectors: change of sign at north fold ONLY if on the local grid
         srcv(ntypsbc,jpr_ocx1:jpr_ocy1)%nsgn = -1.
         ...
         DO jn = 1, jprcv
            IF( srcv(ntypsbc,jn)%clname(1:1) == "O" ) srcv(ntypsbc,jn)%clname = "S"//srcv(ntypsbc,jn)%clname(2:LEN(srcv(ntypsbc,jn)%clname))
         END DO
         !
         IF(lwp) THEN                        ! control print
            WRITE(numout,*)
      ...
      ! =================================================== !
      DO jn = 1, jprcv
         IF( srcv(ntypsbc,jn)%laction ) ALLOCATE( frcv(jn)%z3(jpi,jpj,srcv(ntypsbc,jn)%nct) )
      END DO
      ! Allocate taum part of frcv which is used even when not received as coupling field
      IF( .NOT. srcv(ntypsbc,jpr_taum)%laction ) ALLOCATE( frcv(jpr_taum)%z3(jpi,jpj,srcv(ntypsbc,jpr_taum)%nct) )
      ! Allocate w10m part of frcv which is used even when not received as coupling field
      IF( .NOT. srcv(ntypsbc,jpr_w10m)%laction ) ALLOCATE( frcv(jpr_w10m)%z3(jpi,jpj,srcv(ntypsbc,jpr_w10m)%nct) )
      ! Allocate jpr_otx1 part of frcv which is used even when not received as coupling field
      IF( .NOT. srcv(ntypsbc,jpr_otx1)%laction ) ALLOCATE( frcv(jpr_otx1)%z3(jpi,jpj,srcv(ntypsbc,jpr_otx1)%nct) )
      IF( .NOT. srcv(ntypsbc,jpr_oty1)%laction ) ALLOCATE( frcv(jpr_oty1)%z3(jpi,jpj,srcv(ntypsbc,jpr_oty1)%nct) )
      ! Allocate itx1 and ity1 as they are used in sbc_cpl_ice_tau even if srcv(ntypsbc,jpr_itx1)%laction = .FALSE.
      IF( k_ice /= 0 ) THEN
         IF( .NOT. srcv(ntypsbc,jpr_itx1)%laction ) ALLOCATE( frcv(jpr_itx1)%z3(jpi,jpj,srcv(ntypsbc,jpr_itx1)%nct) )
         IF( .NOT. srcv(ntypsbc,jpr_ity1)%laction ) ALLOCATE( frcv(jpr_ity1)%z3(jpi,jpj,srcv(ntypsbc,jpr_ity1)%nct) )
      ENDIF

      ! ================================ !
      !     Define the send interface    !
      ! ================================ !
      ! for each field: define the OASIS name                           (ssnd(ntypsbc,:)%clname)
      !                 define send or not from the namelist parameters (ssnd(ntypsbc,:)%laction)
      !                 define the north fold type of lbc               (ssnd(ntypsbc,:)%nsgn)

      ! default definitions of nsnd
      ssnd(ntypsbc,:)%laction = .FALSE.   ;   ssnd(ntypsbc,:)%clgrid = 'T'   ;   ssnd(ntypsbc,:)%nsgn = 1.
      ssnd(ntypsbc,:)%nct = 1   ;   ssnd(ntypsbc,:)%nlvl = 1

      !                                                      ! ------------------------- !
      !                                                      !    Surface temperature    !
      !                                                      ! ------------------------- !
      ssnd(ntypsbc,jps_toce)%clname   = 'O_SSTSST'
      ssnd(ntypsbc,jps_tice)%clname   = 'O_TepIce'
      ssnd(ntypsbc,jps_ttilyr)%clname = 'O_TtiLyr'
      ssnd(ntypsbc,jps_tmix)%clname   = 'O_TepMix'
      SELECT CASE( TRIM( sn_snd_temp%cldes ) )
      CASE( 'none'                                 )       ! nothing to do
      CASE( 'oce only'                             )   ;   ssnd(ntypsbc, jps_toce )%laction = .TRUE.
      CASE( 'oce and ice' , 'weighted oce and ice' , 'oce and weighted ice' )
         ssnd(ntypsbc, (/jps_toce, jps_tice/) )%laction = .TRUE.
         IF( TRIM( sn_snd_temp%clcat ) == 'yes' )  ssnd(ntypsbc,jps_tice)%nct = nn_cats_cpl
      CASE( 'mixed oce-ice'                        )   ;   ssnd(ntypsbc, jps_tmix )%laction = .TRUE.
      ...
      !                                                      !          Albedo           !
      !                                                      ! ------------------------- !
      ssnd(ntypsbc,jps_albice)%clname = 'O_AlbIce'
      ssnd(ntypsbc,jps_albmix)%clname = 'O_AlbMix'
      SELECT CASE( TRIM( sn_snd_alb%cldes ) )
      CASE( 'none'                 )     ! nothing to do
      CASE( 'ice' , 'weighted ice' )   ; ssnd(ntypsbc,jps_albice)%laction = .TRUE.
      CASE( 'mixed oce-ice'        )   ; ssnd(ntypsbc,jps_albmix)%laction = .TRUE.
      ...
      !                                                      ! ------------------------- !
      ssnd(ntypsbc,jps_fice)%clname  = 'OIceFrc'
      ssnd(ntypsbc,jps_ficet)%clname = 'OIceFrcT'
      ssnd(ntypsbc,jps_hice)%clname  = 'OIceTck'
      ssnd(ntypsbc,jps_a_p)%clname   = 'OPndFrc'
      ssnd(ntypsbc,jps_ht_p)%clname  = 'OPndTck'
      ssnd(ntypsbc,jps_hsnw)%clname  = 'OSnwTck'
      ssnd(ntypsbc,jps_fice1)%clname = 'OIceFrd'
      IF( k_ice /= 0 ) THEN
         ssnd(ntypsbc,jps_fice)%laction  = .TRUE.                 ! if ice treated in the ocean (even in climato case)
         ssnd(ntypsbc,jps_fice1)%laction = .TRUE.                 ! First-order regridded ice concentration, to be used producing atmos-to-ice fluxes (Met Office requirement)
! Currently no namelist entry to determine sending of multi-category ice fraction so use the thickness entry for now
         IF( TRIM( sn_snd_thick%clcat  ) == 'yes' ) ssnd(ntypsbc,jps_fice)%nct  = nn_cats_cpl
         IF( TRIM( sn_snd_thick1%clcat ) == 'yes' ) ssnd(ntypsbc,jps_fice1)%nct = nn_cats_cpl
      ENDIF

      IF(TRIM( sn_snd_ifrac%cldes )  == 'coupled') ssnd(ntypsbc,jps_ficet)%laction = .TRUE.

      SELECT CASE ( TRIM( sn_snd_thick%cldes ) )
      CASE( 'none'         )       ! nothing to do
      CASE( 'ice and snow' )
         ssnd(ntypsbc,jps_hice:jps_hsnw)%laction = .TRUE.
         IF( TRIM( sn_snd_thick%clcat ) == 'yes' ) THEN
            ssnd(ntypsbc,jps_hice:jps_hsnw)%nct = nn_cats_cpl
         ENDIF
      CASE ( 'weighted ice and snow' )
         ssnd(ntypsbc,jps_hice:jps_hsnw)%laction = .TRUE.
         IF( TRIM( sn_snd_thick%clcat ) == 'yes' ) ssnd(ntypsbc,jps_hice:jps_hsnw)%nct = nn_cats_cpl
      ...
      CASE ( 'none' )
         ssnd(ntypsbc,jps_a_p)%laction  = .FALSE.
         ssnd(ntypsbc,jps_ht_p)%laction = .FALSE.
      CASE ( 'ice only' )
         ssnd(ntypsbc,jps_a_p)%laction  = .TRUE.
         ssnd(ntypsbc,jps_ht_p)%laction = .TRUE.
         IF( TRIM( sn_snd_mpnd%clcat ) == 'yes' ) THEN
            ssnd(ntypsbc,jps_a_p)%nct  = nn_cats_cpl
            ssnd(ntypsbc,jps_ht_p)%nct = nn_cats_cpl
         ELSE
      ...
      ssnd(ntypsbc,jps_ocx1)%clname = 'O_OCurx1'   ;   ssnd(ntypsbc,jps_ivx1)%clname = 'O_IVelx1'
      ssnd(ntypsbc,jps_ocy1)%clname = 'O_OCury1'   ;   ssnd(ntypsbc,jps_ivy1)%clname = 'O_IVely1'
      ssnd(ntypsbc,jps_ocz1)%clname = 'O_OCurz1'   ;   ssnd(ntypsbc,jps_ivz1)%clname = 'O_IVelz1'
      ssnd(ntypsbc,jps_ocxw)%clname = 'O_OCurxw'
      ssnd(ntypsbc,jps_ocyw)%clname = 'O_OCuryw'
      !
      ssnd(ntypsbc,jps_ocx1:jps_ivz1)%nsgn = -1.   ! vectors: change of the sign at the north fold

      IF( sn_snd_crt%clvgrd == 'U,V' ) THEN
         ssnd(ntypsbc,jps_ocx1)%clgrid = 'U' ; ssnd(ntypsbc,jps_ocy1)%clgrid = 'V'
      ELSE IF( sn_snd_crt%clvgrd /= 'T' ) THEN
         CALL ctl_stop( 'sn_snd_crt%clvgrd must be equal to T' )
      ENDIF
      ssnd(ntypsbc,jps_ocx1:jps_ivz1)%laction = .TRUE.   ! default: all are send
      IF( TRIM( sn_snd_crt%clvref ) == 'spherical' )   ssnd(ntypsbc, (/jps_ocz1, jps_ivz1/) )%laction = .FALSE.
      IF( TRIM( sn_snd_crt%clvor ) == 'eastward-northward' ) ssnd(ntypsbc,jps_ocx1:jps_ivz1)%nsgn = 1.
      SELECT CASE( TRIM( sn_snd_crt%cldes ) )
      CASE( 'none'                 )   ;   ssnd(ntypsbc,jps_ocx1:jps_ivz1)%laction = .FALSE.
      CASE( 'oce only'             )   ;   ssnd(ntypsbc,jps_ivx1:jps_ivz1)%laction = .FALSE.
      CASE( 'weighted oce and ice' )   !   nothing to do
      CASE( 'mixed oce-ice'        )   ;   ssnd(ntypsbc,jps_ivx1:jps_ivz1)%laction = .FALSE.
      CASE default   ;   CALL ctl_stop( 'sbc_cpl_init: wrong definition of sn_snd_crt%cldes' )
      END SELECT

      ssnd(ntypsbc,jps_ocxw:jps_ocyw)%nsgn = -1.   ! vectors: change of the sign at the north fold

      IF( sn_snd_crtw%clvgrd == 'U,V' ) THEN
         ssnd(ntypsbc,jps_ocxw)%clgrid = 'U' ; ssnd(ntypsbc,jps_ocyw)%clgrid = 'V'
      ELSE IF( sn_snd_crtw%clvgrd /= 'T' ) THEN
         CALL ctl_stop( 'sn_snd_crtw%clvgrd must be equal to T' )
      ENDIF
      IF( TRIM( sn_snd_crtw%clvor ) == 'eastward-northward' ) ssnd(ntypsbc,jps_ocxw:jps_ocyw)%nsgn = 1.
      SELECT CASE( TRIM( sn_snd_crtw%cldes ) )
         CASE( 'none'                 )   ; ssnd(ntypsbc,jps_ocxw:jps_ocyw)%laction = .FALSE.
         CASE( 'oce only'             )   ; ssnd(ntypsbc,jps_ocxw:jps_ocyw)%laction = .TRUE.
         CASE( 'weighted oce and ice' )   !   nothing to do
         CASE( 'mixed oce-ice'        )   ; ssnd(ntypsbc,jps_ivx1:jps_ivz1)%laction = .FALSE.
      ...
      !                                                      !          CO2 flux         !
      !                                                      ! ------------------------- !
      ssnd(ntypsbc,jps_co2)%clname = 'O_CO2FLX' ;  IF( TRIM(sn_snd_co2%cldes) == 'coupled' )    ssnd(ntypsbc,jps_co2 )%laction = .TRUE.
      !
      !                                                      ! ------------------------- !
      !                                                      ! Sea surface freezing temp !
      !                                                      ! ------------------------- !
      ! needed by Met Office
      ssnd(ntypsbc,jps_sstfrz)%clname = 'O_SSTFrz' ; IF( TRIM(sn_snd_sstfrz%cldes) == 'coupled' )  ssnd(ntypsbc,jps_sstfrz)%laction = .TRUE.
      !
      !                                                      ! ------------------------- !
      !                                                      !    Ice conductivity       !
      !                                                      ! ------------------------- !
      ...
      CASE ( 'none' )
         ssnd(ntypsbc,jps_ttilyr)%laction = .FALSE.
      CASE ( 'ice only' )
         ssnd(ntypsbc,jps_ttilyr)%laction = .TRUE.
         IF( TRIM( sn_snd_ttilyr%clcat ) == 'yes' ) THEN
            ssnd(ntypsbc,jps_ttilyr)%nct = nn_cats_cpl
         ELSE
            IF( nn_cats_cpl > 1 ) THEN
      ...
      CASE ( 'weighted ice' )
         ssnd(ntypsbc,jps_ttilyr)%laction = .TRUE.
         IF( TRIM( sn_snd_ttilyr%clcat ) == 'yes' ) ssnd(ntypsbc,jps_ttilyr)%nct = nn_cats_cpl
      CASE default   ;   CALL ctl_stop( 'sbc_cpl_init: wrong definition of sn_snd_ttilyr%cldes;'//sn_snd_ttilyr%cldes )
      END SELECT

      ssnd(ntypsbc,jps_kice )%clname = 'OIceKn'
      SELECT CASE ( TRIM( sn_snd_cond%cldes ) )
      CASE ( 'none' )
         ssnd(ntypsbc,jps_kice)%laction = .FALSE.
      CASE ( 'ice only' )
         ssnd(ntypsbc,jps_kice)%laction = .TRUE.
         IF( TRIM( sn_snd_cond%clcat ) == 'yes' ) THEN
            ssnd(ntypsbc,jps_kice)%nct = nn_cats_cpl
         ELSE
            IF( nn_cats_cpl > 1 ) THEN
      ...
      CASE ( 'weighted ice' )
         ssnd(ntypsbc,jps_kice)%laction = .TRUE.
         IF( TRIM( sn_snd_cond%clcat ) == 'yes' ) ssnd(ntypsbc,jps_kice)%nct = nn_cats_cpl
      ...
      !                                                      !     Sea surface height    !
      !                                                      ! ------------------------- !
      ssnd(ntypsbc,jps_wlev)%clname = 'O_Wlevel' ;  IF( TRIM(sn_snd_wlev%cldes) == 'coupled' )   ssnd(ntypsbc,jps_wlev)%laction = .TRUE.

      !                                                      ! ------------------------------- !
      !                                                      !   OCE-SAS coupling - snd by opa !
      !                                                      ! ------------------------------- !
      ssnd(ntypsbc,jps_ssh   )%clname = 'O_SSHght'
      ssnd(ntypsbc,jps_soce  )%clname = 'O_SSSal'
      ssnd(ntypsbc,jps_e3t1st)%clname = 'O_E3T1st'
      ssnd(ntypsbc,jps_fraqsr)%clname = 'O_FraQsr'
      !
      IF( nn_components == jp_iam_oce ) THEN
         ssnd(ntypsbc,:)%laction = .FALSE.   ! force default definition in case of opa <-> sas coupling
         ssnd(ntypsbc, (/jps_toce, jps_soce, jps_ssh, jps_fraqsr, jps_ocx1, jps_ocy1/) )%laction = .TRUE.
         ssnd(ntypsbc, jps_e3t1st )%laction = .NOT.ln_linssh
         ! vector definition: not used but cleaner...
         ssnd(ntypsbc,jps_ocx1)%clgrid  = 'U'        ! oce components given at U-point
         ssnd(ntypsbc,jps_ocy1)%clgrid  = 'V'        !           and           V-point
      ...
      !                                                      ! ------------------------------- !
      ssnd(ntypsbc,jps_sflx  )%clname = 'I_SFLX'
      ssnd(ntypsbc,jps_fice2 )%clname = 'IIceFrc'
      ssnd(ntypsbc,jps_qsroce)%clname = 'I_QsrOce'
      ssnd(ntypsbc,jps_qnsoce)%clname = 'I_QnsOce'
      ssnd(ntypsbc,jps_oemp  )%clname = 'IOEvaMPr'
      ssnd(ntypsbc,jps_otx1  )%clname = 'I_OTaux1'
      ssnd(ntypsbc,jps_oty1  )%clname = 'I_OTauy1'
      ssnd(ntypsbc,jps_rnf   )%clname = 'I_Runoff'
      ssnd(ntypsbc,jps_taum  )%clname = 'I_TauMod'
      !
      IF( nn_components == jp_iam_sas ) THEN
         IF( .NOT. ln_cpl ) ssnd(ntypsbc,:)%laction = .FALSE.   ! force default definition in case of opa <-> sas coupling
         ssnd(ntypsbc, (/jps_qsroce, jps_qnsoce, jps_oemp, jps_fice2, jps_sflx, jps_otx1, jps_oty1, jps_taum/) )%laction = .TRUE.
         !
         ...
         DO jn = 1, jpsnd
            IF( ssnd(ntypsbc,jn)%clname(1:1) == "O" ) ssnd(ntypsbc,jn)%clname = "S"//ssnd(ntypsbc,jn)%clname(2:LEN(ssnd(ntypsbc,jn)%clname))
         END DO
         !
         IF(lwp) THEN                        ! control print
            WRITE(numout,*)
      ...
      ! ================================= !
      CALL cpl_var(jprcv, jpsnd, nn_cplmodel, ntypsbc)
   ...
   SUBROUTINE sbc_cpl_rcv( kt, k_fsbc, k_ice, Kbb, Kmm )
      ...
      !   cannot be done in the init phase when we use agrif as cpl_freq requires that oasis_enddef is done
         ncpl_qsr_freq = cpl_freq( 'O_QsrOce', ntypsbc ) + cpl_freq( 'O_QsrMix', ntypsbc ) + &
                     &   cpl_freq( 'I_QsrOce', ntypsbc ) + cpl_freq( 'I_QsrMix', ntypsbc )
      ...
      DO jn = 1, jprcv                                          ! received fields sent by the atmosphere
         IF( srcv(ntypsbc,jn)%laction )   CALL cpl_rcv( jn, isec, ntypsbc, frcv(jn)%z3, nrcvinfo(jn), xcplmask(:,:,1:nn_cplmodel) )
      END DO

      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_otx1)%laction ) THEN                      !  ocean stress components  !
         !                                                   ! ========================= !
         ...
         IF(  nrcvinfo(jpr_otx1) == OASIS_Rcv ) THEN
            ...
            IF( TRIM( sn_rcv_tau%clvor ) == 'eastward-northward' ) THEN   ! 2 components oriented along the local grid
               ...
               CALL rot_rep( frcv(jpr_otx1)%z3(:,:,1), frcv(jpr_oty1)%z3(:,:,1), srcv(ntypsbc,jpr_otx1)%clgrid, 'en->i', ztx )
               IF( srcv(ntypsbc,jpr_otx2)%laction ) THEN
                  CALL rot_rep( frcv(jpr_otx2)%z3(:,:,1), frcv(jpr_oty2)%z3(:,:,1), srcv(ntypsbc,jpr_otx2)%clgrid, 'en->j', zty )
               ELSE
                  CALL rot_rep( frcv(jpr_otx1)%z3(:,:,1), frcv(jpr_oty1)%z3(:,:,1), srcv(ntypsbc,jpr_otx1)%clgrid, 'en->j', zty )
               ENDIF
            ...
            ENDIF
            !
            IF( srcv(ntypsbc,jpr_otx1)%clgrid == 'T' ) THEN
               DO_2D( 0, 0, 0, 0 )                                        ! T ==> (U,V)
      ...
      !                                                      ! ========================= !
      IF( .NOT. srcv(ntypsbc,jpr_taum)%laction ) THEN                    ! compute wind stress module from its components if not received
      ...
      !                                                      ! ========================= !
      IF( .NOT. srcv(ntypsbc,jpr_w10m)%laction ) THEN                    ! compute wind spreed from wind stress module if not received
...
!!$
      ...
      !                                                      ! atmosph. CO2 (ppm) !
      !                                                      ! ================== !
      IF( srcv(ntypsbc,jpr_co2)%laction )   atm_co2(:,:) = frcv(jpr_co2)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      ! Mean Sea Level Pressure   !   (taum)
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_mslp)%laction ) THEN                    ! UKMO SHELF effect of atmospheric pressure on SSH
      ...
      !                                                      !       Stokes drift u      !
      !                                                      ! ========================= !
         IF( srcv(ntypsbc,jpr_sdrftx)%laction ) ut0sd(:,:) = frcv(jpr_sdrftx)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !       Stokes drift v      !
      !                                                      ! ========================= !
         IF( srcv(ntypsbc,jpr_sdrfty)%laction ) vt0sd(:,:) = frcv(jpr_sdrfty)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !      Wave mean period     !
      !                                                      ! ========================= !
         IF( srcv(ntypsbc,jpr_wper)%laction ) wmp(:,:) = frcv(jpr_wper)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !  Significant wave height  !
      !                                                      ! ========================= !
         IF( srcv(ntypsbc,jpr_hsig)%laction ) hsw(:,:) = frcv(jpr_hsig)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !    Vertical mixing Qiao   !
      !                                                      ! ========================= !
         IF( srcv(ntypsbc,jpr_wnum)%laction .AND. ln_zdfswm ) wnum(:,:) = frcv(jpr_wnum)%z3(:,:,1)

         ! Calculate the 3D Stokes drift both in coupled and not fully uncoupled mode
         IF( srcv(ntypsbc,jpr_sdrftx)%laction .OR. srcv(ntypsbc,jpr_sdrfty)%laction .OR. &
             srcv(ntypsbc,jpr_wper)%laction .OR. srcv(ntypsbc,jpr_hsig)%laction )   THEN
            CALL sbc_stokes( Kmm )
         ENDIF
      ENDIF
      !                                                      ! ========================= !
      ...
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_wstrf)%laction .AND. ln_tauoc )  tauoc_wave(:,:) = frcv(jpr_wstrf)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !   Wave drag coefficient   !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_wdrag)%laction .AND. ln_cdgw )   cdn_wave(:,:) = frcv(jpr_wdrag)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !   Chranock coefficient    !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_charn)%laction .AND. ln_charn )  charn(:,:) = frcv(jpr_charn)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      ! net wave-supported stress !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_tawx)%laction .AND. ln_taw )     tawx(:,:) = frcv(jpr_tawx)%z3(:,:,1)
      IF( srcv(ntypsbc,jpr_tawy)%laction .AND. ln_taw )     tawy(:,:) = frcv(jpr_tawy)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !wave to ocean momentum flux!
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_twox)%laction .AND. ln_taw )     twox(:,:) = frcv(jpr_twox)%z3(:,:,1)
      IF( srcv(ntypsbc,jpr_twoy)%laction .AND. ln_taw )     twoy(:,:) = frcv(jpr_twoy)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !    wave TKE flux at sfc   !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_phioc)%laction .AND. ln_phioc )     phioc(:,:) = frcv(jpr_phioc)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !      Bernoulli head       !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_bhd)%laction .AND. ln_bern_srfc )   bhd_wave(:,:) = frcv(jpr_bhd)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !   Stokes transport u dir  !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_tusd)%laction .AND. ln_breivikFV_2016 )    tusd(:,:) = frcv(jpr_tusd)%z3(:,:,1)
      !
      !                                                      ! ========================= !
      !                                                      !   Stokes transport v dir  !
      !                                                      ! ========================= !
      IF( srcv(ntypsbc,jpr_tvsd)%laction .AND. ln_breivikFV_2016 )     tvsd(:,:) = frcv(jpr_tvsd)%z3(:,:,1)
      !
      ...
      !                                                      !        SSS         !
      !                                                      ! ================== !
      IF( srcv(ntypsbc,jpr_soce)%laction ) THEN                      ! received by sas in case of opa <-> sas coupling
      ...
      !                                                      !        SST         !
      !                                                      ! ================== !
      IF( srcv(ntypsbc,jpr_toce)%laction ) THEN                      ! received by sas in case of opa <-> sas coupling
         sst_m(:,:) = frcv(jpr_toce)%z3(:,:,1)
         IF( srcv(ntypsbc,jpr_soce)%laction .AND. l_useCT ) THEN    ! make sure that sst_m is the potential temperature
      ...
      !                                                      !        SSH         !
      !                                                      ! ================== !
      IF( srcv(ntypsbc,jpr_ssh )%laction ) THEN                      ! received by sas in case of opa <-> sas coupling
      ...
      !                                                      !  surface currents  !
      !                                                      ! ================== !
      IF( srcv(ntypsbc,jpr_ocx1)%laction ) THEN                      ! received by sas in case of opa <-> sas coupling
      ...
      ENDIF
      IF( srcv(ntypsbc,jpr_ocy1)%laction ) THEN
      ...
      !                                                      ! ======================== !
      IF( srcv(ntypsbc,jpr_e3t1st )%laction ) THEN                   ! received by sas in case of opa <-> sas coupling
      ...
      !                                                      ! ================================ !
      IF( srcv(ntypsbc,jpr_fraqsr)%laction ) THEN                    ! received by sas in case of opa <-> sas coupling
      ...
      IF( k_ice <= 1 .AND. MOD( kt-1, k_fsbc ) == 0 ) THEN   !  heat & freshwater fluxes ! (Ocean only case)
         ...
         IF( srcv(ntypsbc,jpr_oemp)%laction .OR. srcv(ntypsbc,jpr_rain)%laction ) THEN
         ...
         IF( srcv(ntypsbc,jpr_rnf)%laction )     rnf(:,:) = frcv(jpr_rnf)%z3(:,:,1)
         IF( srcv(ntypsbc,jpr_cal)%laction )     zemp(:,:) = zemp(:,:) - frcv(jpr_cal)%z3(:,:,1)

         IF( srcv(ntypsbc,jpr_icb)%laction )  THEN
         ...
         IF(      srcv(ntypsbc,jpr_qnsoce)%laction ) THEN   ;   zqns(:,:) = frcv(jpr_qnsoce)%z3(:,:,1)
         ELSE IF( srcv(ntypsbc,jpr_qnsmix)%laction ) THEN   ;   zqns(:,:) = frcv(jpr_qnsmix)%z3(:,:,1)
         ELSE                                       ;   zqns(:,:) = 0._wp
         ENDIF
         ! update qns over the free ocean with:
         IF( nn_components /= jp_iam_oce ) THEN
            zqns(:,:) =  zqns(:,:) - zemp(:,:) * sst_m(:,:) * rcp         ! remove heat content due to mass flux (assumed to be at SST)
            IF( srcv(ntypsbc,jpr_snow  )%laction ) THEN
               zqns(:,:) = zqns(:,:) - frcv(jpr_snow)%z3(:,:,1) * rLfus   ! energy for melting solid precipitation over the free ocean
            ENDIF
         ENDIF
         !
         IF( srcv(ntypsbc,jpr_icb)%laction )  zqns(:,:) = zqns(:,:) - frcv(jpr_icb)%z3(:,:,1) * rLfus ! remove heat content associated to iceberg melting
         !
         ...
         IF     ( srcv(ntypsbc,jpr_qsroce)%laction ) THEN   ;   zqsr(:,:) = frcv(jpr_qsroce)%z3(:,:,1)
         ELSE IF( srcv(ntypsbc,jpr_qsrmix)%laction ) then   ;   zqsr(:,:) = frcv(jpr_qsrmix)%z3(:,:,1)
         ELSE                                       ;   zqsr(:,:) = 0._wp
         ENDIF
         ...
         IF( srcv(ntypsbc,jpr_sflx )%laction )   sfx(:,:) = frcv(jpr_sflx  )%z3(:,:,1)
         ! Ice cover  (received by opa in case of opa <-> sas coupling)
         IF( srcv(ntypsbc,jpr_fice )%laction )   fr_i(:,:) = frcv(jpr_fice )%z3(:,:,1)
         !
      ENDIF
      !
   ...
   SUBROUTINE sbc_cpl_ice_tau( p_taui, p_tauj )
...
#if defined key_si3 || defined key_cice
      !
      IF( srcv(ntypsbc,jpr_itx1)%laction ) THEN   ;   itx =  jpr_itx1
      ELSE                                ;   itx =  jpr_otx1
      ENDIF
      ...
      IF(  nrcvinfo(itx) == OASIS_Rcv ) THEN
         ...
         IF( srcv(ntypsbc,jpr_itx1)%laction ) THEN                      !   ice stress received   !
            ...
            IF( TRIM( sn_rcv_tau%clvor ) == 'eastward-northward' ) THEN   ! 2 components oriented along the local grid
               !                                                       ! (geographical to local grid -> rotate the components)
               CALL rot_rep( frcv(jpr_itx1)%z3(:,:,1), frcv(jpr_ity1)%z3(:,:,1), srcv(ntypsbc,jpr_itx1)%clgrid, 'en->i', ztx )
               IF( srcv(ntypsbc,jpr_itx2)%laction ) THEN
                  CALL rot_rep( frcv(jpr_itx2)%z3(:,:,1), frcv(jpr_ity2)%z3(:,:,1), srcv(ntypsbc,jpr_itx2)%clgrid, 'en->j', zty )
               ELSE
                  CALL rot_rep( frcv(jpr_itx1)%z3(:,:,1), frcv(jpr_ity1)%z3(:,:,1), srcv(ntypsbc,jpr_itx1)%clgrid, 'en->j', zty )
               ENDIF
         ...
         !                                                              i-1  i   i
         !                                                               i      i+1 (for I)
         SELECT CASE ( srcv(ntypsbc,jpr_itx1)%clgrid )
         CASE( 'U' )
   ...
   SUBROUTINE sbc_cpl_ice_flx( kt, picefr, palbi, psst, pist, phs, phi )
      ...
      ! --- Continental fluxes --- !
      IF( srcv(ntypsbc,jpr_rnf)%laction ) THEN   ! runoffs (included in emp later on)
         rnf(:,:) = frcv(jpr_rnf)%z3(:,:,1)
      ENDIF
      IF( srcv(ntypsbc,jpr_cal)%laction ) THEN   ! calving (put in emp_tot and emp_oce)
         zemp_tot(:,:) = zemp_tot(:,:) - frcv(jpr_cal)%z3(:,:,1)
         zemp_oce(:,:) = zemp_oce(:,:) - frcv(jpr_cal)%z3(:,:,1)
      ENDIF
      IF( srcv(ntypsbc,jpr_icb)%laction ) THEN   ! iceberg added to runoffs
         fwficb(:,:) = frcv(jpr_icb)%z3(:,:,1)
         rnf(:,:)    = rnf(:,:) + fwficb(:,:)
      ENDIF
      IF( srcv(ntypsbc,jpr_isf)%laction ) THEN   ! iceshelf (fwfisf > 0 mean melting)
...
!!$         rnf(:,:) = frcv(jpr_rnf)%z3(:,:,1)
!!$      ENDIF
!!$      IF( srcv(ntypsbc,jpr_cal)%laction ) THEN   ! calving (put in emp_tot)
!!$         zemp_tot(:,:) = zemp_tot(:,:) - frcv(jpr_cal)%z3(:,:,1)
!!$      ENDIF
!!$      IF( srcv(ntypsbc,jpr_icb)%laction ) THEN   ! iceberg added to runoffs
!!$         fwficb(:,:) = frcv(jpr_icb)%z3(:,:,1)
...
!!$      ENDIF
      !
      ! outputs
      IF( srcv(ntypsbc,jpr_cal)%laction )    CALL iom_put( 'calving_cea' , frcv(jpr_cal)%z3(:,:,1) * tmask(:,:,1)                )  ! calving
      IF( srcv(ntypsbc,jpr_icb)%laction )    CALL iom_put( 'iceberg_cea' , frcv(jpr_icb)%z3(:,:,1) * tmask(:,:,1)                )  ! icebergs
...
!!      IF( srcv(ntypsbc,jpr_rnf)%laction )   CALL iom_put( 'runoffs' , rnf(:,:) * tmask(:,:,1)                                 )  ! runoff
!!      IF( srcv(ntypsbc,jpr_isf)%laction )   CALL iom_put( 'iceshelf_cea', fwfisf(:,:) * tmask(:,:,1)                         )  ! iceshelf
      !
      !                                                      ! ========================= !
...
! ** NEED TO SORT OUT HOW THIS SHOULD WORK IN THE MULTI-CATEGORY CASE - CURRENTLY NOT ALLOWED WHEN INTERFACE INITIALISED **
      ...
      IF( srcv(ntypsbc,jpr_cal)%laction )   zqns_tot(:,:) = zqns_tot(:,:) - frcv(jpr_cal)%z3(:,:,1) * rLfus  ! remove latent heat of calving
                                                                                                     ! we suppose it melts at 0deg, though it should be temp. of surrounding ocean
      ! --- iceberg (removed from qns_tot) --- !
      IF( srcv(ntypsbc,jpr_icb)%laction )   zqns_tot(:,:) = zqns_tot(:,:) - frcv(jpr_icb)%z3(:,:,1) * rLfus  ! remove latent heat of iceberg melting
...
!!$      ENDIF
      ...
      ! outputs
      IF ( srcv(ntypsbc,jpr_cal)%laction ) CALL iom_put('hflx_cal_cea' , - frcv(jpr_cal)%z3(:,:,1) * rLfus ) ! latent heat from calving
      IF ( srcv(ntypsbc,jpr_icb)%laction ) CALL iom_put('hflx_icb_cea' , - frcv(jpr_icb)%z3(:,:,1) * rLfus ) ! latent heat from icebergs melting
...
!!         END SELECT
      ...
      !                                                      ! ================== !
      ! needed by Met Office
      IF( srcv(ntypsbc,jpr_ts_ice)%laction ) THEN
   ...
   SUBROUTINE sbc_cpl_snd( kt, Kbb, Kmm )
      ...
      !                                                      ! ------------------------- !
      IF( ssnd(ntypsbc,jps_toce)%laction .OR. ssnd(ntypsbc,jps_tice)%laction .OR. ssnd(ntypsbc,jps_tmix)%laction ) THEN
         ...
         ELSE
            ...
            END SELECT
         ENDIF
         IF( ssnd(ntypsbc,jps_toce)%laction )   CALL cpl_snd( jps_toce, isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
         IF( ssnd(ntypsbc,jps_tice)%laction )   CALL cpl_snd( jps_tice, isec, ntypsbc, ztmp3, info )
         IF( ssnd(ntypsbc,jps_tmix)%laction )   CALL cpl_snd( jps_tmix, isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
      ENDIF
      !
      !                                                      ! ------------------------- !
...
#if defined key_si3
      ! needed by  Met Office
      IF( ssnd(ntypsbc,jps_ttilyr)%laction) THEN
         ...
         END SELECT
         IF( ssnd(ntypsbc,jps_ttilyr)%laction )   CALL cpl_snd( jps_ttilyr, isec, ntypsbc, ztmp3, info )
      ENDIF
#endif
      !                                                      ! ------------------------- !
      !                                                      !           Albedo          !
      !                                                      ! ------------------------- !
      IF( ssnd(ntypsbc,jps_albice)%laction ) THEN                         ! ice
         ...
         SELECT CASE( sn_snd_alb%clcat )
            CASE( 'yes' )
               CALL cpl_snd( jps_albice, isec, ntypsbc, ztmp3, info )      !-> MV this has never been checked in coupled mode
            CASE( 'no'  )
               CALL cpl_snd( jps_albice, isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
         END SELECT
      ENDIF

      IF( ssnd(ntypsbc,jps_albmix)%laction ) THEN                         ! mixed ice-ocean
         ztmp1(:,:) = alb_oce_mix(:,:) * zfr_l(:,:)
         DO jl = 1, jpl
            ztmp1(:,:) = ztmp1(:,:) + alb_ice(:,:,jl) * a_i(:,:,jl)
         END DO
         CALL cpl_snd( jps_albmix, isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
      ENDIF
      !                                                      ! ------------------------- !
      !                                                      !  Ice fraction & Thickness !
      !                                                      ! ------------------------- !
      ! Send ice fraction field to atmosphere
      IF( ssnd(ntypsbc,jps_fice)%laction ) THEN
         ...
         END SELECT
         CALL cpl_snd( jps_fice, isec, ntypsbc, ztmp3, info )
      ENDIF
      ...
      IF( ssnd(ntypsbc,jps_fice1)%laction ) THEN
         SELECT CASE( sn_snd_thick1%clcat )
         CASE( 'yes' )   ;   ztmp3(:,:,1:jpl) =  a_i(:,:,1:jpl)
         CASE( 'no'  )   ;   ztmp3(:,:,1    ) = fr_i(:,:      )
         CASE default    ;   CALL ctl_stop( 'sbc_cpl_snd: wrong definition of sn_snd_thick1%clcat' )
         END SELECT
         CALL cpl_snd( jps_fice1, isec, ntypsbc, ztmp3, info )
      ENDIF

      ! Send ice fraction field to OCE (sent by SAS in SAS-OCE coupling)
      IF( ssnd(ntypsbc,jps_fice2)%laction ) THEN
         ztmp3(:,:,1) = fr_i(:,:)
         IF( ssnd(ntypsbc,jps_fice2)%laction )   CALL cpl_snd( jps_fice2, isec, ntypsbc, ztmp3, info )
      ENDIF

      ! Send ice and snow thickness field
      IF( ssnd(ntypsbc,jps_hice)%laction .OR. ssnd(ntypsbc,jps_hsnw)%laction ) THEN
         ...
         END SELECT
         IF( ssnd(ntypsbc,jps_hice)%laction )   CALL cpl_snd( jps_hice, isec, ntypsbc, ztmp3, info )
         IF( ssnd(ntypsbc,jps_hsnw)%laction )   CALL cpl_snd( jps_hsnw, isec, ntypsbc, ztmp4, info )
      ENDIF
      ...
      IF( ssnd(ntypsbc,jps_a_p)%laction .OR. ssnd(ntypsbc,jps_ht_p)%laction ) THEN
         ...
         END SELECT
         IF( ssnd(ntypsbc,jps_a_p)%laction  )   CALL cpl_snd( jps_a_p , isec, ntypsbc, ztmp3, info )
         IF( ssnd(ntypsbc,jps_ht_p)%laction )   CALL cpl_snd( jps_ht_p, isec, ntypsbc, ztmp4, info )
      ENDIF
      !
      !                                                      ! ------------------------- !
      !                                                      !     Ice conductivity      !
      !                                                      ! ------------------------- !
      ! needed by Met Office
      IF( ssnd(ntypsbc,jps_kice)%laction ) THEN
         ...
         END SELECT
         IF( ssnd(ntypsbc,jps_kice)%laction )   CALL cpl_snd( jps_kice, isec, ntypsbc, ztmp3, info )
      ENDIF
#endif
      ...
      !                                                      ! ------------------------- !
      IF( ssnd(ntypsbc,jps_co2)%laction .AND. l_co2cpl )   THEN
         ztmp1(:,:) = oce_co2(:,:) * 1000.  ! conversion in molC/m2/s
         CALL cpl_snd( jps_co2, isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ) , info )
      ENDIF
      !
      !                                                      ! ------------------------- !
      IF( ssnd(ntypsbc,jps_ocx1)%laction ) THEN                      !      Surface current      !
         !                                                   ! ------------------------- !
         !
         !                                                  j+1   j     -----V---F
         ...
         ELSE
            ...
            CASE( 'mixed oce-ice'        )      ! Ocean and Ice on C-grid ==> T
               ...
               END_2D
            END SELECT
            CALL lbc_lnk( 'sbccpl', zotx1, ssnd(ntypsbc,jps_ocx1)%clgrid, -1.0_wp,  zoty1, ssnd(ntypsbc,jps_ocy1)%clgrid, -1.0_wp )
            !
         ENDIF
         !
         !
         IF( TRIM( sn_snd_crt%clvor ) == 'eastward-northward' ) THEN             ! Rotation of the components
            !                                                                     ! Ocean component
            CALL rot_rep( zotx1, zoty1, ssnd(ntypsbc,jps_ocx1)%clgrid, 'ij->e', ztmp1 )       ! 1st component
            CALL rot_rep( zotx1, zoty1, ssnd(ntypsbc,jps_ocx1)%clgrid, 'ij->n', ztmp2 )       ! 2nd component
            zotx1(:,:) = ztmp1(:,:)                                                   ! overwrite the components
            zoty1(:,:) = ztmp2(:,:)
            IF( ssnd(ntypsbc,jps_ivx1)%laction ) THEN                                     ! Ice component
               CALL rot_rep( zitx1, zity1, ssnd(ntypsbc,jps_ivx1)%clgrid, 'ij->e', ztmp1 )    ! 1st component
               CALL rot_rep( zitx1, zity1, ssnd(ntypsbc,jps_ivx1)%clgrid, 'ij->n', ztmp2 )    ! 2nd component
         ...
         IF( TRIM( sn_snd_crt%clvref ) == 'cartesian' ) THEN
            ...
            !
            IF( ssnd(ntypsbc,jps_ivx1)%laction ) THEN           ! ice velocities
               ztmp1(:,:) = zitx1(:,:)
               ztmp1(:,:) = zity1(:,:)
         ...
         ENDIF
         !
         IF( ssnd(ntypsbc,jps_ocx1)%laction )   CALL cpl_snd( jps_ocx1, isec, ntypsbc, RESHAPE ( zotx1, (/jpi,jpj,1/) ), info )   ! ocean x current 1st grid
         IF( ssnd(ntypsbc,jps_ocy1)%laction )   CALL cpl_snd( jps_ocy1, isec, ntypsbc, RESHAPE ( zoty1, (/jpi,jpj,1/) ), info )   ! ocean y current 1st grid
         IF( ssnd(ntypsbc,jps_ocz1)%laction )   CALL cpl_snd( jps_ocz1, isec, ntypsbc, RESHAPE ( zotz1, (/jpi,jpj,1/) ), info )   ! ocean z current 1st grid
         !
         IF( ssnd(ntypsbc,jps_ivx1)%laction )   CALL cpl_snd( jps_ivx1, isec, ntypsbc, RESHAPE ( zitx1, (/jpi,jpj,1/) ), info )   ! ice   x current 1st grid
         IF( ssnd(ntypsbc,jps_ivy1)%laction )   CALL cpl_snd( jps_ivy1, isec, ntypsbc, RESHAPE ( zity1, (/jpi,jpj,1/) ), info )   ! ice   y current 1st grid
         IF( ssnd(ntypsbc,jps_ivz1)%laction )   CALL cpl_snd( jps_ivz1, isec, ntypsbc, RESHAPE ( zitz1, (/jpi,jpj,1/) ), info )   ! ice   z current 1st grid
         !
      ENDIF
      !
      !                                                      ! ------------------------- !
      ...
      !                                                      ! ------------------------- !
      IF( ssnd(ntypsbc,jps_ocxw)%laction .OR. ssnd(ntypsbc,jps_ocyw)%laction ) THEN
          !
          !                                                  j+1  j     -----V---F
          ! surface velocity always sent from T point                    !       |
          !                                                       j      |   T   U
          ...
          CASE( 'mixed oce-ice'        )      ! Ocean and Ice on C-grid ==> T
             ...
             END_2D
          END SELECT
         CALL lbc_lnk( 'sbccpl', zotx1, ssnd(ntypsbc,jps_ocxw)%clgrid, -1.0_wp, zoty1, ssnd(ntypsbc,jps_ocyw)%clgrid, -1.0_wp )
         !
         !
         IF( TRIM( sn_snd_crtw%clvor ) == 'eastward-northward' ) THEN             ! Rotation of the components
         !                                                                        ! Ocean component
            CALL rot_rep( zotx1, zoty1, ssnd(ntypsbc,jps_ocxw)%clgrid, 'ij->e', ztmp1 )       ! 1st component
            CALL rot_rep( zotx1, zoty1, ssnd(ntypsbc,jps_ocxw)%clgrid, 'ij->n', ztmp2 )       ! 2nd component
            zotx1(:,:) = ztmp1(:,:)                                                   ! overwrite the components
            zoty1(:,:) = ztmp2(:,:)
            IF( ssnd(ntypsbc,jps_ivx1)%laction ) THEN                                     ! Ice component
               CALL rot_rep( zitx1, zity1, ssnd(ntypsbc,jps_ivx1)%clgrid, 'ij->e', ztmp1 )    ! 1st component
...
!         ENDIF
         !
         IF( ssnd(ntypsbc,jps_ocxw)%laction )   CALL cpl_snd( jps_ocxw, isec, ntypsbc, RESHAPE ( zotx1, (/jpi,jpj,1/) ), info )   ! ocean x current 1st grid
         IF( ssnd(ntypsbc,jps_ocyw)%laction )   CALL cpl_snd( jps_ocyw, isec, ntypsbc, RESHAPE ( zoty1, (/jpi,jpj,1/) ), info )   ! ocean y current 1st grid
         !
      ENDIF
      !
      IF( ssnd(ntypsbc,jps_ficet)%laction ) THEN
         CALL cpl_snd( jps_ficet, isec, ntypsbc, RESHAPE ( fr_i, (/jpi,jpj,1/) ), info )
      ENDIF
      !                                                      ! ------------------------- !
      !                                                      !   Water levels to waves   !
      !                                                      ! ------------------------- !
      IF( ssnd(ntypsbc,jps_wlev)%laction ) THEN
         IF( ln_apr_dyn ) THEN
            IF( kt /= nit000 ) THEN
         ...
         ELSE
            ztmp1(:,:) = ssh(:,:,Kmm)
         ENDIF
         CALL cpl_snd( jps_wlev  , isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info )
      ENDIF
      !
      !  Fields sent by OCE to SAS when doing OCE<->SAS coupling
      !                                                        ! SSH
      IF( ssnd(ntypsbc,jps_ssh )%laction )  THEN
         ...
         ELSE                    ;   ztmp1(:,:) = ssh(:,:,Kmm)
         ENDIF
         CALL cpl_snd( jps_ssh   , isec, ntypsbc, RESHAPE ( ztmp1            , (/jpi,jpj,1/) ), info )

      ENDIF
      !                                                        ! SSS
      IF( ssnd(ntypsbc,jps_soce  )%laction )  THEN
         CALL cpl_snd( jps_soce  , isec, ntypsbc, RESHAPE ( ts(:,:,1,jp_sal,Kmm), (/jpi,jpj,1/) ), info )
      ENDIF
      !                                                        ! first T level thickness
      IF( ssnd(ntypsbc,jps_e3t1st )%laction )  THEN
 CALL cpl_snd( jps_e3t1st, isec, ntypsbc, RESHAPE ( e3t(:,:,1,Kmm) , (/jpi,jpj,1/) ), info )
      ENDIF
      !                                                        ! Qsr fraction
      IF( ssnd(ntypsbc,jps_fraqsr)%laction )  THEN
         CALL cpl_snd( jps_fraqsr, isec, ntypsbc, RESHAPE ( fraqsr_1lev(:,:) , (/jpi,jpj,1/) ), info )
      ENDIF
      !
      !  Fields sent by SAS to OCE when OASIS coupling
      !                                                        ! Solar heat flux
      IF( ssnd(ntypsbc,jps_qsroce)%laction )  CALL cpl_snd( jps_qsroce, isec, ntypsbc, RESHAPE ( qsr , (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_qnsoce)%laction )  CALL cpl_snd( jps_qnsoce, isec, ntypsbc, RESHAPE ( qns , (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_oemp  )%laction )  CALL cpl_snd( jps_oemp  , isec, ntypsbc, RESHAPE ( emp , (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_sflx  )%laction )  CALL cpl_snd( jps_sflx  , isec, ntypsbc, RESHAPE ( sfx , (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_otx1  )%laction )  CALL cpl_snd( jps_otx1  , isec, ntypsbc, RESHAPE ( utau, (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_oty1  )%laction )  CALL cpl_snd( jps_oty1  , isec, ntypsbc, RESHAPE ( vtau, (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_rnf   )%laction )  CALL cpl_snd( jps_rnf   , isec, ntypsbc, RESHAPE ( rnf , (/jpi,jpj,1/) ), info )
      IF( ssnd(ntypsbc,jps_taum  )%laction )  CALL cpl_snd( jps_taum  , isec, ntypsbc, RESHAPE ( taum, (/jpi,jpj,1/) ), info )
      ...
      ztmp1(:,:) = sstfrz(:,:) + rt0
      IF( ssnd(ntypsbc,jps_sstfrz)%laction )  CALL cpl_snd( jps_sstfrz, isec, ntypsbc, RESHAPE ( ztmp1, (/jpi,jpj,1/) ), info)
#endif
      !
