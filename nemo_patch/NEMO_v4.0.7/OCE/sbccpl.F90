...
!!       WRITE(numout,*)'      transmitted solar thru sea-ice  = ', TRIM(sn_rcv_qtrice%cldes), ' (', TRIM(sn_rcv_qtrice%clcat), ')'
      ...
      srcv(ntypsbc,:)%laction = .FALSE.   ;   srcv(ntypsbc,:)%clgrid = 'T'   ;   srcv(ntypsbc,:)%nsgn = 1.   ;   srcv(ntypsbc,:)%nct = 1
      ...
      IF( srcv(ntypsbc,jpr_isf)%laction .AND. ln_isf ) THEN
      ...
      lhftau = srcv(ntypsbc,jpr_taum)%laction
      !
      !                                                      ! ------------------------- !
      !                                                      !      Atmospheric CO2      !
      !                                                      ! ------------------------- !
...
!!      ENDIF
      ...
      IF( TRIM(sn_rcv_wper%cldes  ) == 'coupled' )  THEN
         ...
         cpl_wper = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_wfreq)%clname = 'O_WFreq'     ! wave peak frequency 
      IF( TRIM(sn_rcv_wfreq%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_wfreq)%laction = .TRUE.
         cpl_wfreq = .TRUE.
      ENDIF
      ...
      IF( TRIM(sn_rcv_wnum%cldes ) == 'coupled' )  THEN
         ...
         cpl_wnum = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_tauwoc)%clname = 'O_TauOce'   ! stress fraction adsorbed by the wave
      IF( TRIM(sn_rcv_tauwoc%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_tauwoc)%laction = .TRUE.
         cpl_tauwoc = .TRUE.
      ENDIF
      srcv(ntypsbc,jpr_tauwx)%clname = 'O_Tauwx'      ! ocean stress from wave in the x direction
      srcv(ntypsbc,jpr_tauwy)%clname = 'O_Tauwy'      ! ocean stress from wave in the y direction
      IF( TRIM(sn_rcv_tauw%cldes ) == 'coupled' )  THEN
         srcv(ntypsbc,jpr_tauwx)%laction = .TRUE.
         srcv(ntypsbc,jpr_tauwy)%laction = .TRUE.
         cpl_tauw = .TRUE.
      ENDIF
      ...
      IF( TRIM(sn_rcv_wdrag%cldes ) == 'coupled' )  THEN
         ...
         cpl_wdrag = .TRUE.
      ENDIF
      IF( srcv(ntypsbc,jpr_tauwoc)%laction .AND. srcv(ntypsbc,jpr_tauwx)%laction .AND. srcv(ntypsbc,jpr_tauwy)%laction ) &
      ...
      ssnd(ntypsbc,:)%laction = .FALSE.   ;   ssnd(ntypsbc,:)%clgrid = 'T'   ;   ssnd(ntypsbc,:)%nsgn = 1.  ; ssnd(ntypsbc,:)%nct = 1
...
! Currently no namelist entry to determine sending of multi-category ice fraction so use the thickness entry for now
   ...
   SUBROUTINE sbc_cpl_rcv( kt, k_fsbc, k_ice )     
      ...
      !   cannot be done in the init phase when we use agrif as cpl_freq requires that oasis_enddef is done
         ncpl_qsr_freq = cpl_freq( 'O_QsrOce', ntypsbc ) + cpl_freq( 'O_QsrMix', ntypsbc ) + cpl_freq( 'I_QsrOce',ntypsbc ) + cpl_freq( 'I_QsrMix', ntypsbc )
...
!!$
      ...
      !                                                      !    Wave peak frequency    ! 
      !                                                      ! ========================= !  
         IF( srcv(ntypsbc,jpr_wfreq)%laction ) wfreq(:,:) = frcv(jpr_wfreq)%z3(:,:,1)
      !
      !                                                      ! ========================= ! 
      ...
      !                                                      ! ========================= ! 
         ...
         IF( srcv(ntypsbc,jpr_sdrftx)%laction .OR. srcv(ntypsbc,jpr_sdrfty)%laction .OR. srcv(ntypsbc,jpr_wper)%laction &
                                      .OR. srcv(ntypsbc,jpr_hsig)%laction   .OR. srcv(ntypsbc,jpr_wfreq)%laction) THEN
            CALL sbc_stokes()
         ENDIF
      ENDIF
      !                                                      ! ========================= ! 
      ...
      !                                                      ! ========================= ! 
      IF( srcv(ntypsbc,jpr_tauwoc)%laction .AND. ln_tauwoc ) tauoc_wave(:,:) = frcv(jpr_tauwoc)%z3(:,:,1)

      !                                                      ! ========================= !  
      !                                                      ! Stress component by waves ! 
      !                                                      ! ========================= !  
      IF( srcv(ntypsbc,jpr_tauwx)%laction .AND. srcv(ntypsbc,jpr_tauwy)%laction .AND. ln_tauw ) THEN
      ...
      IF( k_ice <= 1 .AND. MOD( kt-1, k_fsbc ) == 0 ) THEN   !  heat & freshwater fluxes ! (Ocean only case)
         ...
         ENDIF
         IF( srcv(ntypsbc,jpr_isf)%laction )  fwfisf(:,:) = - frcv(jpr_isf)%z3(:,:,1)  ! fresh water flux from the isf (fwfisf <0 mean melting)  
   ...
   SUBROUTINE sbc_cpl_snd( kt )
      ...
      IF( ssnd(ntypsbc,jps_ocx1)%laction ) THEN                      !      Surface current      !
         ...
         ELSE        
            ...
            CASE( 'mixed oce-ice'        )      ! Ocean and Ice on C-grid ==> T
               ...
               END DO
            END SELECT
            CALL lbc_lnk_multi( 'sbccpl', zotx1, ssnd(ntypsbc,jps_ocx1)%clgrid, -1.,  zoty1, ssnd(ntypsbc,jps_ocy1)%clgrid, -1. )
            !
         ENDIF
         !
         !
      ...
      IF( ssnd(ntypsbc,jps_ocxw)%laction .OR. ssnd(ntypsbc,jps_ocyw)%laction ) THEN 
          ...
          CASE( 'mixed oce-ice'        )      ! Ocean and Ice on C-grid ==> T  
             ...
             END DO
          END SELECT
         CALL lbc_lnk_multi( 'sbccpl', zotx1, ssnd(ntypsbc,jps_ocxw)%clgrid, -1., zoty1, ssnd(ntypsbc,jps_ocyw)%clgrid, -1. ) 
         ! 
         ! 
...
!         ENDIF 
      ...
      IF( ssnd(ntypsbc,jps_soce  )%laction )  THEN
         CALL cpl_snd( jps_soce  , isec, ntypsbc, RESHAPE ( tsn(:,:,1,jp_sal), (/jpi,jpj,1/) ), info )
      ENDIF
      !                                                        ! first T level thickness 
      IF( ssnd(ntypsbc,jps_e3t1st )%laction )  THEN
         CALL cpl_snd( jps_e3t1st, isec, ntypsbc, RESHAPE ( e3t_n(:,:,1)   , (/jpi,jpj,1/) ), info )
      ENDIF
      !                                                        ! Qsr fraction
